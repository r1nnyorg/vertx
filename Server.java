public class Server
{
    public static void main(final java.lang.String[] args)
    {
        ...
        router.post("/ajax").respond(ctx -> database.query("select * from" + ctx.getBodyAsJson().stream().map($ -> java.lang.String.join(" ", $.getKey(), $.getValue().toString())).collect(java.util.stream.Collectors.joining(" "))).execute().map(rowset -> java.util.stream.StreamSupport.stream(rowset.spliterator(), false).findFirst().orElse("haha")  )  );  
