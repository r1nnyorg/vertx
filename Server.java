        ...
        router.post("/ajax").respond(ctx ->
{
    ...
    return client.connect().compose(connection -> redis.get(body)).compose(records -> java.util.Objects.isNull(records) ? database.query("select * from" + body).execute().map(rowset -> java.util.stream.StreamSupport.stream(rowset.spliterator(), false).collect(java.util.stream.Collectors.toList())) : io.vertx.core.Future.succeededFuture(records));
});  
