public class Server
{
	...
	final var route = org.springframework.web.reactive.function.server.RouterFunctions.route(org.springframework.web.reactive.function.server.RequestPredicates.GET("/"), request -> org.springframework.web.reactive.function.server.ServerResponse.ok().bodyValue(new com.fasterxml.jackson.databind.ObjectMapper().writeValueAsString(client.sql("select * from productItem").fetch().all().toStream().collect(java.util.stream.Collectors.toList())) ));
