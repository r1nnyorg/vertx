public class Server
{
	...
	final var route = org.springframework.web.reactive.function.server.RouterFunctions.route(org.springframework.web.reactive.function.server.RequestPredicates.POST("/ajax"), request -> request.bodyToMono(java.lang.String.class).map(body -> org.springframework.web.reactive.function.server.ServerResponse.ok().body(client.sql(body).fetch().all(), java.util.Map.class)));
